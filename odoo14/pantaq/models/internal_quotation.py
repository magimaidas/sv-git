...
class InternalOrder(models.Model):
    ...
    def _send_mail_notify(self, action='submit'):
        ...
        SalesPartners = list(map(lambda x: x.partner_id.id, self.lead_id.sudo().team_id.member_ids)) #used sudo() as PE doesn't have read access to Enquiries that aren't assigned.
        ProcPartners = list(map(lambda x: x.partner_id.id, self.lead_id.sudo().team_proc_id.member_ids)) #used sudo() as PE doesn't have read access to Enquiries that aren't assigned.
        NotifyPartners = []
    ...
    def _prepare_CustQuote(self):
        ...
        vals.update({'origin' : case.lead_id.sudo().enq_number, #used sudo() as SE doesn't have read access to Enquiries that are hidden.
                     'sale_type'  : 'quote',
                     'state':'draft',
    ...
    def action_create_CustQuote(self):
        ...
        for case in self:
            ...
            if cq:
                ...
                vals.update({'user_id': self._uid}) # Updates Sales Person (user_id) with current user id
                newRec = sale_obj.create(vals)
                CQids.append(newRec.id)
        print("BBBBBBBBBBBB",newRec)
        print("CCCCCCCCCCCCCCCCCCCCCCC",vals)
    ...
    def create(self, vals):
        ...
        if vals.get('lead_id', False):
            ...
            if StageID:
                self.env['crm.lead'].browse(vals['lead_id']).sudo().with_context(ctx).write({'stage_id': StageID.id})
...
class InternalOrderLine(models.Model):
    ...
    def _visible_approval(self):
        ...
        for case in self:
            ...
            if case.approval_state in ('awaiting', 'rejected'):
                Aflag = True

            if case.approval_state in ('awaiting', 'approved'):
                Rflag = True
    ...
    rfqline_id  = fields.Many2one('purchase.order', string='RFQ Product Reference', readonly=True)
    currency_id = fields.Many2one('res.currency', store=True, string='Currency', readonly=True, help='Currency is quoted w.r.t RFQ')
    # enqline_id  = fields.Many2one(related='rfqline_id.enqline_id', relation='pq.enquiry.lines', store=True, string='Enquiry Product')
    ...
    target_currency_id = fields.Many2one(relation='res.currency', store=True, string='Target Price Currency', readonly=True)
    target_price = fields.Float(string='Target Cost', store=True, readonly=True, digits=dp.get_precision('Product Price'))
    ...
    approval_state = fields.Selection([
                ('awaiting', 'Awaiting'),
                ('approved', 'Approved'),
                ('rejected', 'Rejected'),], string='Approval Status', readonly=True, copy=False)
    ...
    def onchange_product(self):
        ...
        vals = {}
        # domain = {'product_uom': [('category_id', '=', self.product_id.product_tmpl_id.uom_id.category_id.id)]}
        if not self.product_uom or (self.product_id.product_tmpl_id.uom_id.category_id.id != self.product_uom.category_id.id):
    ...
    def _prepare_IQLines_CQLines(self):
        ...
        for case in self:
            ...
            for field in ['name', 'product_id', 'product_uom_qty', 'product_uom']:
                if case._fields[field].type == 'many2one':
                    vals[field] = case[field].id
                else:
                    vals[field] = case[field] or False
            ...
            if includeTax:
                ...
                print("If condition",price)
            else:
                price = case.price_subtotal
                print("else condition",price)
            ...
            if case.currency_id:
                ...
                price_cost = 0
            print("After condition",case.price_cost)
            vals.update({'price_cost': case.price_cost,
                         'price_unit': case.price_cost,
