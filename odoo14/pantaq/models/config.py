...
class Partner(models.Model):
    ...
    vendor_type = fields.Selection([('manufacturer', 'Manufacturer'), ('reseller', 'Reseller'),('supplier','Supplier')], string="Vendor Type", default='reseller')
...
class Message(models.Model):
    ...
    def _get_UserGroups(self):
        ...
        for author_id in self.author_id:
            AuthUser = self.env['res.users'].search([('partner_id', '=', author_id.id)])
            if AuthUser:
                PurchaseGrp = AuthUser.user_has_groups('purchase.group_purchase_user')
                SalesGrp = AuthUser.user_has_groups('sales_team.group_sale_salesman')
